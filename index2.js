...
console.log("------------------------------------");
console.log("Interval Run started");
var run=0;
...
function fullRun() {
 	getTweets('@tenxwallet when ?');
 	getTweets('@julianhosp when ?');
	retweetTenX();
}
...
async function getTweets(query) {
	...
	return T.get('search/tweets', params, function (err, data) {})
	.then(
		function(data) {
			 tweets = data.data.statuses;
			 tweetsToReply = [];
			 for (tweet in tweets) {
			 	if (isTenX(tweets[tweet].user.id_str)) {continue;}
			 	currentTweet = tweets[tweet];
			 	if (region = hasCountry(currentTweet.text)) {
			 		response = '@' + currentTweet.user.screen_name + ' ' + region[1] + ' is ' + isSupported(region) + ' region. First cards will ship to Singapore, followed by APAC, Europe and the rest of the World. Stay tuned for further announcements. Get involved at https://chat.tenx.tech';
			 		tweetsToReply[currentTweet.id_str] = response;
			 	} else if (hasKeyword(currentTweet.text)) {
			 		response = '@' + currentTweet.user.screen_name + ' ' + 'Currently there is no set timeframe for "When cards". Stay tuned for further announcements. Get involved at https://chat.tenx.tech';
			 		tweetsToReply[currentTweet.id_str] = response;
			 	}
			 }
			 replyToTweets(tweetsToReply);
		}
	);
}

function replyToTweets(tweets) {
	newReplied = [];
	for (tweet in tweets) {
		// console.log('Status: ' + tweets[tweet]);
		// console.log('---------------------');
		if (notRepliedYet(tweet) && (newReplied.indexOf(tweet) == '-1')) {
			console.log('New Reply to Tweet ' + tweet);
...
function hasKeyword(tweet) {
	...
	for (word in tweet) {
		tweet[word] = deleteLineEnding(tweet[word]);
		for (keyword in keywords) {
			if (tweet[word].toLowerCase() == keywords[keyword]) {
				return true;
			}
		}
	}
	...
	for (word in tweet) {
		...
		for (var c =  0; c < 2; c++) {
		  ...
		  for (l = 0; l < searchCountries.length; l++) {
		    ...
		    if (tweet[word].toLowerCase() == searchCountries[l].toLowerCase()) {
...
async function checkTweetsFromTenx() {
  ...
  await T.get('statuses/user_timeline', { user_id: '4585412124', count: 100, include_rts: false, exclude_replies: true }, function(err, data, response) {
    for (var t =  0; t < data.length; t++) {
      ...
      var searchArray = ['card', 'cards']
      var retweet = false;

      //Check if Substrings are found in string0
      for (l = 0; l < searchArray.length; l++) {
        //logger.info(substringArray[l]);
        if (data[t].text.indexOf(searchArray[l]) > -1 && data[t].retweeted == false) {
          retweet = true;
...
async function retweetTenX() {
  retweetIDs = await checkTweetsFromTenx();
  // console.log(retweetIDs);
  for (var m = 0; m < retweetIDs.length; m++) {
    T.post('statuses/retweet/:id', { id: retweetIDs[m] }, function (err, data, response) {
      // console.log(data);
    }); 
  }
};

function deleteLineEnding(str) {
	var lineEndings = ['?', '!', '.'];
	for (ending in lineEndings) {
		if (str.indexOf(lineEndings[ending]) != -1) {
			str = str.substring(0, str.indexOf(lineEndings[ending]));
		}
	}
	return str;
}
